...
windows = [5, 8, 11]
seed = 10 ## seed for /data folder
# seed = 7000  ## seed for /data_rep2 folder
seed = 12000 ## seed for /data_rep3 folder
n_genomes = 100
...
for alpha in alphas:
    ...
    ref_genome = generate_random_dna(10000, seed=seed)
    
    # Step 2: Generate related genomes with mutations
    related_genomes = [mutate_sequence(ref_genome, sub_rate=alpha, indel_rate=0.00, seed=seed + i) for i in range(n_genomes-1)]
    
    # Step 3: Write genomes to FASTA files
    dir_name = f"/home/Users/rdd4/mft-analyses/Figure2/data/alpha_{alpha}"
    dir_name = f"/home/Users/rdd4/mft-analyses/Figure2/data_rep2/alpha_{alpha}"
    dir_name = f"/home/Users/rdd4/mft-analyses/Figure2/data_rep3/alpha_{alpha}"
    ...
    for w in windows:
        ...
        for i, baseline_fasta in enumerate(fasta_file_locations):
            ...
            if not os.path.exists(out_transfomed):
                subprocess.run([
                    "cargo", "run", "--",
                    "transform",
                    "-w", str(w),
