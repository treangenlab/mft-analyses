...
RESULTS_ROOT = "/dodo/rdd4/mft-data/seq-gen-data-cov_parsnp"
TRUTH_ROOT   = "/dodo/rdd4/mft-data/seq-gen-data-cov"
...
def load_truth_set(folder_name):
    """Return frozenset of (pos, ref, alt) tuples from truth_snps.vcf."""
    path = os.path.join(TRUTH_ROOT, folder_name, "truth_snps.vcf")
    if not os.path.exists(path):
        return frozenset()
    snps = set()
    with open(path) as f:
        ...
        for line in f:
            ...
            if len(parts) < 3:
                continue
            snps.add((int(parts[0]), parts[1], parts[2]))
    return frozenset(snps)


def load_parsnp_set(vcf_path):
    """Return frozenset of (pos, ref, alt) tuples from a parsnp VCF.
    ALT field may be comma-separated; each allele is added separately."""
    if not os.path.exists(vcf_path):
        return frozenset()
    snps = set()
    with open(vcf_path) as f:
        for line in f:
            ...
            if len(parts) < 5:
                continue
            pos, ref = int(parts[1]), parts[3]
            for alt in parts[4].split(","):
                snps.add((pos, ref, alt))
    return frozenset(snps)
...
rows = []
folder_re = re.compile(r"^(\d+)_([\d.]+)_rep(\d+)$")
...
for folder in sorted(os.listdir(RESULTS_ROOT)):
    ...
    truth_set = load_truth_set(folder)
    ...
    for ds in datasets:
        ...
        parsnp_set = load_parsnp_set(vcf_post_path)
        tp = len(parsnp_set & truth_set)
        fp = len(parsnp_set - truth_set)
        fn = len(truth_set - parsnp_set)
        ...
        rows.append({
            ...
            "fp":          fp,
            "fn":          fn,
            "truth_snps":  len(truth_set),
        })

    out_csv = os.path.join(folder_path, "parsnp_summary_short.csv")
    folder_rows = [r for r in rows if r["n"] == n_val and r["alpha"] == alpha and r["rep"] == rep]
