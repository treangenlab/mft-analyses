...
def write_truth_vcf(dataset_dir):
    ...
    ref_path = os.path.join(nuc_dir, "genome_0.fasta")
    if not os.path.exists(ref_path):
        print(f"  SKIP (no genome_0.fasta): {dataset_dir}")
        return
    ...
    genome_files = sorted(
        glob.glob(os.path.join(nuc_dir, "genome_*.fasta")),
        key=lambda p: int(Path(p).stem.split("_")[1]),
    )
    genome_files = [f for f in genome_files if Path(f).stem != "genome_0"]
    ...
    for col_idx, (gfile, gname) in enumerate(zip(genome_files, genome_names)):
